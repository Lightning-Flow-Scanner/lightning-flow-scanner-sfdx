import Scan, { ScanResult } from '../../../src/commands/flow/scan.js'
...
describe('flow:scan', () => {
  ...
  it('throw error if --directory and --sourcepath are given', async () => {
    ...
    try {
      const output = await new Scan(['--directory', 'test/', '--sourcepath', 'test/'], config).run();
      console.log(output);
    } catch (e) {
      expect((e as Error).message).to.include('You can only specify one of either directory or sourcepath, not both.');
    }
